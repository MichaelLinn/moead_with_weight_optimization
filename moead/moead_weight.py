...
class moead_w(object):
    ...
    def tchebysheff_constrains_func(self, lambda_, individual, i_subproblem):
        # constrain is w's l2 norm == 1
        # transfer the constrain problem to the non-constrain problem using ( beta_ * constrain), beta_ is infinity
        beta_ = 10e5
        rho_ = 10e-6
        #　print "constrain: ", rho_ * - sum(np.log(np.array(lambda_)+ 10e-3  ))
        result = self.tchebysheff_func(lambda_, individual, i_subproblem)  + rho_ * -sum(np.log(np.array(lambda_) + 10e-5))\
                 + beta_ * float(np.mat(lambda_) * np.mat(lambda_).T - 1)
        return result
    ...
    def searchArmijoWeight(self, gradient, weight, new_solution, i_subproblem):
        ...
        while True:
            ...
            func_2 = self.tchebysheff_func(weight, new_solution, i_subproblem) + rho_ * (beta_ ** m) * float(- np.mat(gradient) * np.mat(gradient).T)
